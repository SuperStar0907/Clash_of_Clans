...
if __name__ == "__main__":
    ...
    set_buildings(0,board,ff)
    board.no_of_buildings=11
    cursor.hide()
    board.k=king()
    board.k.health=100
    board.k.show_king(board)
    ...
    while(1):
        print('\033[35;38H'+"   ")
        print('\033[35;38H'+str(board.k.health))
        print('\033[35;61H'+"  ")
        print('\033[35;61H'+str(board.no_of_buildings))
        p= input_to(cursor,speed)
        # print('\033[1;1H')'
        if p == None:
            pass
        elif p=='q':
            ...
            s=Spell("Fire",board,board.k,b)
            speed=0.05
            # fire spell 
        elif p=='h':
            os.system('afplay ./src/sounds/coin.mp3')
            s=Spell("Health",board,board.k,b)
            # health spell
        elif p=='l':
            l= board.k.levi(board)
        else:
            l = board.k.move(p,board)
        if(board.no_of_buildings==0):
            ...
            t2= Thread(target=secondFunction(board,board.k,b))
            t1.start()
            t2.start()
        if(p==None):
            f.write(str('-'))
            ...
            f.write(str(p))
    set_buildings(1,board,ff)
    print('\033[35;38H'+"   ")
    print('\033[35;38H'+str(int(board.k.health/3)))
cursor.show()
f.close()
